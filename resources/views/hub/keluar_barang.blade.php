...
<div class="container-fluid mt--6">
                    ...
                    <table class="table align-items-center table-flush" id="tableData">
                        <thead class="thead-light">
                          <tr>
                            ...
                            <th scope="col" class="sort" data-sort="budget">Driver</th>
                            <th scope="col" class="sort" data-sort="budget">Telepon</th>
                            <th scope="col" class="sort" data-sort="status">Plat Kendaraan</th>
                            <th scope="col" class="sort" data-sort="status">Jumlah (Liter)</th>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Konfirmasi Barang Keluar</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form method="post" id="formConfirm" class="form-horizontal" enctype="multipart/form-data">
                        ...
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-primary"  name="confirm_button" id="confirm_button">Konfirmasi</button>
                        </div>
                    </form>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        function loadData(limit, page){
            $.ajax({
                url: 'loadDataBarangHub/'+limit+'/'+page,
                type: 'get',
                dataType: 'json',
                        ...
                        if(len > 0){
                            ...
                            for(var i=0; i<len; i++){
                                if (response['data'][i].status == 'pending') {
                                  var button = "<a href='#' class='table-action confirm' data-toggle='tooltip' title='Edit Data' id="+response['data'][i].id+" driver="+response['data'][i].nama_driver+" volume="+response['data'][i].jumlah_liter+"><i class='fas fa-edit'></i></a>";
                                } else {
                                  var button = "<a href='#' class='table-action' data-toggle='tooltip' title='Info Data'><i class='fas fa-info'></i></a>";
                                }
                                
                                var content = "<tr>" +
                                "<td>" + response['data'][i].created_at + "</td>" +
                                "<td>" + response['data'][i].nama_driver + "</td>" +
                                "<td>" + response['data'][i].phone_driver + "</td>" +
                                "<td>" + response['data'][i].nomor_kendaraan + "</td>" +
                                "<td>" + response['data'][i].jumlah_liter + "</td>" +
        ...
        $(document).on('click', '.confirm', function(){
            ...
            $('#id_barang').val(id);
            $('#text-confirm').html('Apakah Anda ingin melakukan konfirmasi bahwa driver '+driver+' telah mengambil barang sebanyak '+volume+' liter ?');
              ...
              success: function (data) {
                  ...
                  if(data.status==='success') {
                        swal("Sukses!", "Data berhasil disimpan !", "success").then(function() {
                          window.location = "../keluar_barang_hub";
                        });  
                    } else {
