...
<div class="container-fluid mt--6">
        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <h5>Order ID : #{{$detail['order_id']}} </h5>
                </div>
                
                <hr>
                <div class="card-body">
                    <div class="timeline timeline-one-side" data-timeline-content="axis" data-timeline-axis-style="dashed">
                        @if(!empty($detail['order_history']['0']['status']))
                        <div class="timeline-block">
                            <span class="timeline-step badge-success">
                                <i class="ni ni-diamond"></i>
                            </span>
                            <div class="timeline-content">
                                <small class="text-muted font-weight-bold">{{$detail['order_history']['0']['created_at']}}</small>
                                <h5 class="mt-3 mb-0">Pesanan berhasil dibuat</h5>
                                <p class="text-sm mt-1 mb-0">User membuat pesanan baru.</p>
                                <p class="text-sm mt-1 mb-0">Nama : {{$detail['member']['nama']}}</p>
                                <p class="text-sm mt-1 mb-0">Telepon : 0{{$detail['member']['phone']}}</p>

                            </div>
                        </div>
                        @endif
                        @if(!empty($detail['order_history']['1']['status']))
                        <div class="timeline-block">
                            <span class="timeline-step badge-warning">
                                <i class="ni ni-sound-wave"></i>
                            </span>
                            <div class="timeline-content">
                                <small class="text-muted font-weight-bold">{{$detail['order_history']['1']['created_at']}}</small>
                                <h5 class="mt-3 mb-0">{{$detail['order_history']['1']['description']}}</h5>
                                <p class="text-sm mt-1 mb-0">Proses pencarian mitra berdasarkan pesanan user</p>
                            </div>
                        </div>
                        @endif
                        @if(!empty($detail['order_history']['2']['status']))
                        <div class="timeline-block">
                            <span class="timeline-step badge-info">
                                <i class="ni ni-bulb-61"></i>
                            </span>
                            <div class="timeline-content">
                                <small class="text-muted font-weight-bold">{{$detail['order_history']['2']['created_at']}}</small>
                                ...
                                <p class="text-sm mt-1 mb-0">Nama : {{$detail['mitra']['nama']}}</p>
                                <p class="text-sm mt-1 mb-0">Telepon : 0{{$detail['mitra']['phone']}}</p>
                            </div>
                        </div>
                        @endif
                        @if(!empty($detail['order_history']['3']['status']))
                        <div class="timeline-block">
                            <span class="timeline-step badge-danger">
                                <i class="ni ni-delivery-fast"></i>
                            </span>
                            <div class="timeline-content">
                                <small class="text-muted font-weight-bold">{{$detail['order_history']['3']['created_at']}}</small>
                                <h5 class="mt-3 mb-0">{{$detail['order_history']['3']['description']}}</h5>
                                <p class="text-sm mt-1 mb-0">Mitra telah sampai di lokasi user.</p>
                            </div>
                        </div>
                        @endif
                        @if(!empty($detail['order_history']['4']['status']))
                        <div class="timeline-block">
                            <span class="timeline-step badge-success">
                                <i class="ni ni-tag"></i>
                            </span>
                            <div class="timeline-content">
                                <small class="text-muted font-weight-bold">{{$detail['order_history']['4']['created_at']}}</small>
                                <h5 class="mt-3 mb-0">Pesanan berhasil dipickup</h5>
                                <p class="text-sm mt-1 mb-0">Mitra sedang menuju perjalanan ke pool.</p>
                            </div>
                        </div>
                        @endif
                        @if(!empty($detail['order_history']['5']['status']))
                        <div class="timeline-block">
                            <span class="timeline-step badge-warning">
                                <i class="ni ni-shop text-yellow"></i>
                            </span>
                            <div class="timeline-content">
                                <small class="text-muted font-weight-bold">{{$detail['order_history']['5']['created_at']}}</small>
                                ...
                                <p class="text-sm mt-1 mb-0">Nama Pool: {{$detail['hub']['nama']}}</p>
                            </div>
                        </div>
                        @endif
                        @if(!empty($detail['order_history']['6']['status']))
                        <div class="timeline-block">
                            <span class="timeline-step badge-info">
                                <i class="ni ni-check-bold"></i>
                            </span>
                            <div class="timeline-content">
                                <small class="text-muted font-weight-bold">{{$detail['order_history']['6']['created_at']}}</small>
                                ...
                                <div class="mt-3">
                                    <span class="badge badge-pill badge-warning">Verifikasi Jumlah Liter : {{$detail['quantity']}} </span>
                                </div>
                            </div>
                        </div>
                        @endif
    ...
    function initialize() {
        ...
        var valLat  = "{{$detail['hub']['latitude']}}";
        var valLong = "{{$detail['hub']['longitude']}}";
